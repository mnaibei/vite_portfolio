...
const Button: React.FC<ButtonProps> = ({
  ...
  const commonProps = {
    ...
    shadow-xl hover:scale-100 hover:-translate-y-2 hover:drop-shadow-xl transition-all duration-200 lg:w-max sm:w-2/3`,
  };
  ...
  if (elementType === "input") {
    ...
    return (
      <Link to={link || ""} className="no-underline border-4 border-indigo-500">
        <button {...commonProps}>
          {IconSVGComponent ? (
