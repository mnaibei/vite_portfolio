...
const Button: React.FC<ButtonProps> = ({
  ...
  const commonProps = {
    ...
    shadow-xl hover:scale-100 hover:-translate-y-2 hover:drop-shadow-xl transition-all duration-200 lg:w-max sm:w-1/3`,
  };
  ...
  if (elementType === "input") {
    ...
    return (
      <Link
        to={link || ""}
        className="no-underline border-4 border-blue-500 sm:w-1/3 lg:w-max">
        <button {...commonProps}>
          {IconSVGComponent ? (
            <IconSVGComponent
              className={"lg:w-max border-4 border-red-400 h-10"}
            />
          ) : (
            <img
              src={buttoncolor || ""}
